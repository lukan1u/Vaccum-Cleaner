...
*Any device trying to access SMB share that is not part of the same workgroup will be blocked by windows firewall. In the setting for the firewall you will have to add inbound `tcp port 445` that is used by SMB protocol. Otherwise you will not be able to connect.*
...
`net share` is a useful command to check all shared folder on windows system. Computer management software can be used to check the same info 
```cmd-session
