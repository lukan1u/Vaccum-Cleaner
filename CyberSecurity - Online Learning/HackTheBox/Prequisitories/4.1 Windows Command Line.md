...
**Values:**
Values are data objects linked to a specific key in the registry. Each value has a name, type, and the necessary information to describe its purpose
...
#### How do we access information on the registry?
1. One of the ways of accessing the registry is using the `reg.exe` a command line tool that is used to managing registry database. With `reg.exe`, you can:
	- Query or search for specific keys or values in the registry.
	- Add, modify, or delete registry keys and values.
	- Export and import parts of the registry.
1. Another way of accessing the registry is to use `Get-Item`, `Get-ItemProperry`

Get-Item to show currently running processes/services #command 
```powershell-session
...
Better than recursive search #command 
```powershell-session
...
Looking for software in the registry #command 
```powershell-session
reg query HKEY_LOCAL_MACHINE\SOFTWARE\7-Zip
```

Searching for passwords in the registry #command 
```
...
Using reg to accomplish the above:
```powershell
reg add "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\RunOnce\TestKey" /v access /t REG_SZ /d "C:\Users\htb-student\Downloads\payload.exe"  
```

Removing property and the key #command 
...
# Windows Event Log
---
Windows collects log data from various different applications and user interactions. There is a special API that windows event logger that allows applications to manage and maintain own event logs. Event log event is a required service that runs in context of others.
...
[And many more](https://www.ultimatewindowssecurity.com/securitylog/encyclopedia/)

### Windows Log Technical Details
- Service is displayed as `Windows Event Log`
- Runs inside the process of `svchosts.exe` that runs on boot level
- If the service is stopped it can cause high system instability
- Logs are stored: `C:\Windows\System32\winevt\logs`

show event logs #command 
```powershell-session
ls C:\Windows\System32\winevt\logs
```

- Can be accessed via `Windows Event Viewer`
- Command line via utilities such as: `wivutil` & `Get-WinEvent` and can be used on both local and remote machines

### Wevtutil and windows logs
...
All log sources #command 
```cmd-session
wevtutil el
```

Configuration for specific log info #command 
```cmd-session
wevtutil gl "Windows PowerShell"
```

More info from logging #command 
```cmd-session
wevtutil gli "Windows PowerShell"
```

Querry windows events logs #command 
```cmd-session
 wevtutil qe Security /c:5 /rd:true /f:text
```
- **`wevtutil`**: This is a command-line utility for managing Windows event logs.
- **`qe`**: Stands for "query events". This command is used to retrieve events from the event log.
- **`Security`**: Specifies the event log you want to query, in this case, the **Security** log, which contains security-related events.
- **`/c:5`**: Limits the output to the **most recent 5 events** from the Security log.
- **`/rd:true`**: Specifies to **read the events in reverse order**, meaning the latest events will be displayed first.
- **`/f:text`**: Formats the output in **plain text** format, making it easier to read.
...
Listing all logs in powershell #command 
```powershell-session
Get-WinEvent -ListLog *
```

Specific logs in powerhsell #command 
```powershell-session
Get-WinEvent -ListLog Security
```

 Querying Last Five Events #command 
 ```powershell-session
Get-WinEvent -LogName 'Security' -MaxEvents 5 | Select-Object -ExpandProperty Message
```
- **`Get-WinEvent`**: This is a PowerShell cmdlet used to retrieve events from event logs on a Windows system.
- **`-LogName 'Security'`**:
    - **`-LogName`**: Specifies which event log to query.
    - **`'Security'`**: Refers to the **Security event log**, which contains security-related events.
- **`-MaxEvents 5`**: Limits the results to the **5 most recent events** from the Security log.
- **`|`**: This is the **pipeline operator**. It passes the output of the previous command (the events retrieved) to the next command.
- **`Select-Object`**: This cmdlet is used to select specific properties from the objects passed through the pipeline.
- **`-ExpandProperty Message`**:
    - **`-ExpandProperty`**: Indicates that you want to retrieve the values of a specific property from the objects.
    - **`Message`**: Refers to the property of the event that contains the **detailed message** of each event.

Filtering for logon failures #command 
```powershell-session
Get-WinEvent -FilterHashTable @{LogName='Security';ID='4625 '}
```

How to look for systems that crashed #command 
```powershell-session
Get-WinEvent -FilterHashTable @{LogName='System';Level='1'} | select-object -ExpandProperty Message
```
# Networking managment with CLI
---
