...
## Cmdlets
PowerShell is using cmdlets which are small single function tools built into the shell. Bascially it's the commands such as `cd` and `ls`. Cmdlets are in the form **Verb-Noun**. `Get-ChildItem` is used to list current directory. `Get-ChildItem -Recurse`